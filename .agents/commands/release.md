...
Releasing `agent-motion` is automated. Pushing a `v*` tag triggers
...
and regenerate + push `Formula/agent-motion.rb` (with shell completions) to the tap.
The tag also triggers `.github/workflows/publish-skill.yml`, which publishes
`skills/agent-motion` to `shhac/agent-skills`. **No manual build, and no manual
formula bump.**
...
5. Verify CI and the outputs:
   ```bash
   gh run watch --repo shhac/agent-motion          # release + Publish skill runs green
   gh release view "v${new_version}" --repo shhac/agent-motion   # 6 assets
   ```
   Install / upgrade: `brew install shhac/tap/agent-motion` · `brew upgrade shhac/tap/agent-motion`

## Manual fallback (only if the workflow itself is broken)

Re-run a failed release with `gh run rerun <id> --repo shhac/agent-motion`. To bypass
...
`Formula/agent-motion.rb` by hand (see this file's git history for the old full flow).
...
repo-specific "agent-motion release automation (env-scoped)" one). The skill
...
```bash
ssh-keygen -t ed25519 -N "" -C "agent-motion release automation" -f tap_key
gh repo deploy-key add tap_key.pub -R shhac/homebrew-tap --allow-write \
  --title "agent-motion release automation (env-scoped)"
gh secret set TAP_DEPLOY_KEY --repo shhac/agent-motion --env homebrew-tap < tap_key
